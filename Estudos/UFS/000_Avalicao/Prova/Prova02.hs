...
--Questao 01)
{--
Elabore uma função recursiva que receba um caractere e uma string e retorne a
intercalação das letras da string com o caractere dado. Caso a string seja vazia ou
...
ascendentemente retorna a lista eliminando todos os duplicados. A função deve
...
{--
Entrada: lengthLista [[1,2],[],[1,2,3],[3,4,5], logo o resultado eh [2,0,3,3], e o maior Length eh 3] 
...
{--
 Usando a técnica de pedir mais informação ao amigo, defina uma função que
calcule a posição do maior elemento de uma lista de inteiros. As posições começam
a partir de zero. Caso o maior elemento da lista ocorra em várias posições, a função
deve retornar a menor posição. Sua solução não pode utilizar nenhuma função
pré-definida no Prelude nem definir função auxiliar que faça o mesmo que
alguma destas.
--}

questao04 :: [Int] ->  Int
questao04 [] = -1 --Nao existe, uma vez que ha elemento para ter uma posicao
questao04 [x] = 0 --eh o unico elemento, entao a sua posicao so poder ser a zero
